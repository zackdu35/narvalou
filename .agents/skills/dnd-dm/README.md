...
5. **Test It**:
   ```bash
   node scripts/speak-npc.js --text "Welcome, brave adventurer!" --voice wizard --npc "Gandalf"
   ```
...
```bash
# Goblin ambush
node scripts/speak-npc.js --text "You die now, pinkskin!" --voice goblin --npc "Cragmaw Scout"

# Wise wizard
node scripts/speak-npc.js --text "The path ahead is fraught with danger." --voice wizard --npc "Elminster"

# Villain monologue
node scripts/speak-npc.js --text "You fools! You've played right into my hands!" --voice villain --npc "The Black Spider"
```
...
./scripts/roll-dice.sh 2d6+3 --label "Damage"

# Advantage/Disadvantage
./scripts/roll-dice.sh 1d20+3 --advantage --label "Attack with advantage"
./scripts/roll-dice.sh 1d20 --disadvantage --label "Stealth in heavy armor"
...
Files are stored in: `.agents/skills/dnd-dm/resources/sessions/<campaign-name>/`
...
```bash
chmod +x scripts/roll-dice.sh
chmod +x scripts/speak-npc.js
```

## Files

```
.agents/skills/dnd-dm/
